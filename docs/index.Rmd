...
The data used in this section is available [here](https://raw.githubusercontent.com/niwa/Statistics-and-R/github/data/auckland.csv) (right click and "Save As"). This `.csv` file contains the dates and daily avereged temprature values in Auckland (see [this](#reading-in-data) section to import this into your session). We are going to use the `lubridate` package to deal with dates in **R**. To install this package run `install.pacages("lubridate")` (remeber to run `library(lubridate)` before you use the functionality it contains.)
